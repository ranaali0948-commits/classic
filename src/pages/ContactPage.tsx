...
export default function ContactPage() {
  ...
  return <div className="contact-page">
    ...
    <section className="contact-location-section"><div className="container contact-location-grid">
      <article className="contact-location-panel"><p className="section-label">{restaurant.brandName}</p><h2>{restaurant.brandName}</h2><div className="contact-detail"><MapPin /><p><strong>{copy.address}</strong>{restaurant.address.full}</p></div><div className="contact-detail"><Phone /><p><strong>{copy.phone}</strong><a href={restaurant.phone.href}>{restaurant.phone.display}</a></p></div><div className="contact-detail"><Clock3 /><div>{copy.hours.map(hour => <p key={hour}>{hour}</p>)}<small>{copy.hoursNote}</small></div></div><div className="contact-location-actions"><ButtonLink to={restaurant.mapUrl}>{copy.route}</ButtonLink><ButtonLink to={restaurant.phone.href} variant="secondary">{copy.call}</ButtonLink><ButtonLink to="/reservation" variant="secondary">{copy.reserve}</ButtonLink></div></article>
