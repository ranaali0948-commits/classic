...
export default function ContactPage() {
  ...
  const copy = language === 'fr' ? {
    address: 'Adresse', route: 'Itinéraire', call: 'Appeler', reserve: 'Réserver une table', phone: 'Téléphone', hours: ['Service du midi', 'Fermeture l’après-midi', 'Service du soir à partir de 18h'], hoursNote: 'Horaires hebdomadaires à confirmer.',
    mapTitle: `Carte Google Maps de ${BRAND_NAME}, 10 Rue d’Odessa, Paris 14e`, mapFallback: 'Ouvrir l’itinéraire dans Google Maps',
    ...
    notice: 'Merci pour votre message. Pour une réponse immédiate, veuillez appeler le restaurant.',
    ...
    address: 'Address', route: 'Directions', call: 'Call', reserve: 'Book a table', phone: 'Phone', hours: ['Lunch service', 'Closed in the afternoon', 'Evening service from 6pm'], hoursNote: 'Weekly opening hours to be confirmed.',
    mapTitle: `Google Map of ${BRAND_NAME}, 10 Rue d’Odessa, Paris 14th`, mapFallback: 'Open directions in Google Maps',
    ...
    notice: 'Thank you for your message. For an immediate answer, please call the restaurant.',
  ...
  return <div className="contact-page">
    ...
    <section className="contact-location-section"><div className="container contact-location-grid">
      <article className="contact-location-panel"><p className="section-label notranslate" translate="no">{BRAND_NAME}</p><h2 className="notranslate" translate="no">{BRAND_NAME}</h2><div className="contact-detail"><MapPin /><p><strong>{copy.address}</strong><span className="notranslate" translate="no">{restaurant.address.full}</span></p></div><div className="contact-detail"><Phone /><p><strong>{copy.phone}</strong><a className="notranslate" translate="no" href={restaurant.phone.href}>{restaurant.phone.display}</a></p></div><div className="contact-detail"><Clock3 /><div>{copy.hours.map(hour => <p key={hour}>{hour}</p>)}<small>{copy.hoursNote}</small></div></div><div className="contact-location-actions"><ButtonLink to={restaurant.mapUrl}>{copy.route}</ButtonLink><ButtonLink to={restaurant.phone.href} variant="secondary">{copy.call}</ButtonLink><ButtonLink to="/reservation" variant="secondary">{copy.reserve}</ButtonLink></div></article>
    ...
    <section className="contact-form-section"><div className="container contact-form-layout"><div className="contact-form-intro"><p className="section-label">{copy.formEyebrow}</p><h2>{copy.formTitle}</h2><p>{copy.formText}</p><div><strong>{copy.summary}</strong><p>{copy.summaryText}</p><a className="notranslate" translate="no" href={restaurant.phone.href}>{restaurant.phone.display}</a></div></div><form className="form-card contact-form" onSubmit={event => { event.preventDefault(); setMessage(true); }}><label>{copy.name}<input required name="name" autoComplete="name" /></label><label>{copy.email}<input required name="email" type="email" autoComplete="email" /></label><label>{copy.phone}<input name="phone" type="tel" autoComplete="tel" /></label><label>{copy.subject}<input required name="subject" /></label><label className="full-field">{copy.message}<textarea required name="message" rows={6} /></label><DemoSubmitButton>{copy.send}</DemoSubmitButton>{message && <p className="demo-message" role="status">{copy.notice}</p>}</form></div></section>
