...
function Header() {
  ...
  return <header className={`site-header ${solid ? 'is-solid' : ''}`}><div className="container header-inner">
    <Link to="/" className="brand" aria-label={`${restaurant.brandName}, ${language === 'fr' ? 'accueil' : 'home'}`}><Sparkles aria-hidden="true" /><span>{restaurant.brandName}</span><small>{language === 'fr' ? 'Cuisine indienne & pakistanaise · Paris 14e' : 'Indian & Pakistani cuisine · Paris 14th'}</small></Link>
